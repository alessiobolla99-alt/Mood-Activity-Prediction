...
def clean_data(df):
    ...
    if 'full_date' in df.columns:
        ...
        for fmt in date_formats:
            if df['full_date_dt'].isna().all():
                df['full_date_dt'] = pd.to_datetime(
                    df['full_date'], format=fmt, errors='coerce'
                )
        ...
        if df['full_date_dt'].isna().all():
            print("    Using automatic date parser")
            df['full_date_dt'] = pd.to_datetime(
                df['full_date'], errors='coerce', dayfirst=True
            )
        ...
        date_min = df['date_parsed'].min().date()
        date_max = df['date_parsed'].max().date()
        print(f"  Date range: {date_min} to {date_max}")
    else:
        print("  Warning: No 'full_date' column found")
    ...
    print(f"Cleaned: {initial_rows:,} -> {len(df):,} rows ({removed} removed)")
...
def add_temporal_features(df):
    ...
    if 'mood' in df.columns:
        mood_mapping = {
            'Awful': 0, 'Bad': 1, 'Normal': 2,
            'Good': 3, 'Amazing': 4, 'Excellent': 4
        }
    ...
    # Lag features
    median_mood = df['mood_numeric'].median()
    df['mood_lag_1'] = df['mood_numeric'].shift(1).fillna(median_mood)
    df['mood_lag_7'] = df['mood_numeric'].shift(7).fillna(median_mood)
    df['mood_lag_2'] = df['mood_numeric'].shift(2).fillna(median_mood)

    # Rolling statistics
    df['rolling_mean_7d'] = df['mood_numeric'].rolling(
        window=7, min_periods=1
    ).mean()
    df['rolling_std_7d'] = df['mood_numeric'].rolling(
        window=7, min_periods=1
    ).std().fillna(0)
    df['rolling_mean_3d'] = df['mood_numeric'].rolling(
        window=3, min_periods=1
    ).mean()
    df['rolling_min_7d'] = df['mood_numeric'].rolling(
        window=7, min_periods=1
    ).min()

    # MSSD: Mean Squared Successive Difference
    successive_diffs = df['mood_numeric'].diff(1) ** 2
    df['MSSD_7d'] = successive_diffs.rolling(
        window=7, min_periods=1
    ).mean().fillna(0)
    ...
    df['momentum_3d'] = df['first_difference'].rolling(
        window=3, min_periods=1
    ).mean()
    ...
    df['trend_slope_7d'] = df['mood_numeric'].rolling(
        window=7, min_periods=2
    ).apply(calc_slope, raw=True)
    df['month_sin'] = np.sin(2 * np.pi * df['month'] / 12)

    print("  Created 15 temporal features")
...
def add_activity_features(df):
    ...
    for activity in all_activities:
        count = df['activities'].str.contains(
            activity, regex=False, na=False
        ).sum()
        activity_counts[activity] = count

    # Keep top 10 most frequent specific activities
    top_10_activities = sorted(
        activity_counts.items(), key=lambda x: x[1], reverse=True
    )[:10]

    print("  Keeping top 10 most frequent activities")
    feature_count = 0

    # Create binary features for top 10 activities
    for activity, _ in top_10_activities:
        col_name = f"activity_{activity.lower().replace(' ', '_').replace('-', '_')}"
        df[col_name] = df['activities'].str.contains(
            activity, regex=False, na=False
        ).astype(int)
        feature_count += 1

    # Aggregate features
    df['total_activity_count'] = activities_split.apply(
    ...
    df['any_social_activity'] = df['activities'].apply(
        lambda x: int(any(kw in str(x).lower() for kw in social_keywords))
    )
    feature_count += 1
    ...
    df['physical_activity_count'] = df['activities'].apply(
        lambda x: sum(kw in str(x).lower() for kw in physical_keywords)
    )
    feature_count += 1
    ...
    df['self_care_count'] = df['activities'].apply(
        lambda x: sum(kw in str(x).lower() for kw in selfcare_keywords)
    )
    feature_count += 1
...
def add_mood_features(df):
    ...
    Returns:
        pd.DataFrame: Dataset with encoded mood features
    """
    print("\nAdd mood features:")
    ...
    if mood_mapping is not None:
        ...
        with open('data/processed/mood_mapping.json', 'w', encoding='utf-8') as f:
            json.dump({
                'encoding': mood_mapping,
                'decoding': reverse_mapping,
                'type': 'ordinal'
            }, f, indent=2)
        print("  Mood mapping saved to data/processed/mood_mapping.json")
        ...
        n_unique = df[mood_col].nunique()
        print(f"  Using label encoding for {n_unique} unique moods")

        # Save encoder
        os.makedirs('data/processed', exist_ok=True)
        joblib.dump(le, 'data/processed/label_encoder.pkl')
        print("  Label encoder saved to data/processed/label_encoder.pkl")
    ...
    if df['target'].isna().any():
        ...
        print(f"  Warning: {n_missing} missing values in target")
        df = df.dropna(subset=['target'])

    print("Mood encoding complete")
...
def save_processed_data(df, output_dir="data/processed"):
    ...
    temporal_keep = [
        ...
        'rolling_mean_7d', 'rolling_std_7d', 'rolling_mean_3d',
        'rolling_min_7d', 'MSSD_7d',
        'day_of_week_sin', 'day_of_week_cos', 'is_weekend',
        'first_difference', 'momentum_3d', 'trend_slope_7d', 'month_sin'
    ]

    activity_cols = ['total_activity_count', 'any_social_activity',
                     'physical_activity_count', 'self_care_count',
                     'activity_diversity_7d']
    activity_keep = [col for col in df.columns
                     if col.startswith('activity_') or col in activity_cols]
    ...
    n_activity = len(activity_keep)
    print(f"\nProcessed data saved: {output_path}")
    print(f"  Shape: {df_clean.shape[0]:,} rows x {df_clean.shape[1]} columns")
    print(f"  Kept: 15 temporal + {n_activity} activity + 1 target = "
          f"{df_clean.shape[1]} features")
...
def display_info(df):
    ...
    temporal_keywords = ['lag', 'rolling', 'MSSD', 'difference',
                         'momentum', 'slope', 'sin', 'cos', 'weekend']
    temporal_features = [col for col in df.columns
                         if any(x in col for x in temporal_keywords)]

    activity_cols = ['total_activity_count', 'any_social_activity',
                     'physical_activity_count', 'self_care_count',
                     'activity_diversity_7d']
    activity_features = [col for col in df.columns
                         if col.startswith('activity_') or col in activity_cols]
...
def prepare_data(data_dir="data/raw", output_dir="data/processed"):
    ...
    Checks for local dataset first. If not found, downloads from Kaggle.
    This allows the project to run without Kaggle credentials when the
    ...
    if os.path.exists(data_dir):
        ...
        if csv_files:
            csv_path = os.path.join(data_dir, csv_files[0])
            print(f"\nDataset found locally: {csv_files[0]}")

    # If no local dataset, try to download from Kaggle
    if csv_path is None:
        print("\nNo local dataset found, attempting Kaggle download...")
        if not setup_kaggle_credentials():
            print("\nError: Dataset not found and Kaggle credentials missing")
            print("Please either:")
            print("  1. Place the dataset CSV in data/raw/")
            print("  2. Configure Kaggle credentials:")
            print("     - Set KAGGLE_USERNAME and KAGGLE_KEY env variables, OR")
